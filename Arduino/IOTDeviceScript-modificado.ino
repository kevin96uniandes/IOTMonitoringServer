...
#define DHTPIN D7
// Declaramos los pines, tanto del sensor o fotoresistor y el pin del LED
...
const char ssid[] = "MALDONADO DELGADO"; // TODO cambiar por el nombre de la red WiFi
// Contraseña de la red WiFi
const char pass[] = "KD649666"; // TODO cambiar por la contraseña de la red WiFi
...
const char MQTT_HOST[] = "54.167.200.63"; // TODO Reemplace ip.maquina.mqtt por la IP del bróker MQTT que usted desplegó. Ej: 192.168.0.1
...
/**
 * Publica la temperatura, humedad yu luminosidad dadas al tópico configurado usando el cliente MQTT.
 */
...
/**
 * Lee la luminosidad del sensor DHT, la imprime en consola y la devuelve.
 */
int readLuminosidad() {
  // Se lee la humedad relativa
  int lumino = analogRead(LIGHT_SENSOR_PIN);
...
void displayMeasures() {
  ...
  display.println("");
  display.print("    ");
  // Agregamos para que muestre en la pantalla el valor de luminosidad
  display.print("L: ");
  display.print(lumi);
  display.println("");
}
...
String checkAlert() {
  ...
  if (alert.length() != 0) {
    message = alert;
  //Agregamos la sección de codiog para que prenda el LED solo con las alertas de luminocidad
    if (alert.indexOf("LUMINOSIDAD") >= 0) {
      turnOnLed();
    }
...
void turnOnLed() {
  Serial.print("prediendo ...");


  digitalWrite(YELLOW_LED, HIGH);  // Se prende el LED
  delay(60000);             // Espera 1 minuto (60,000 milisegundos)
  digitalWrite(YELLOW_LED, LOW);   // Apaga el LED después de 1 minuto
...
void setup() {
 ...
 // Declaramos el pin del LEd como pun de salida y que el estado inicia del LED sea apago 
  pinMode(YELLOW_LED, OUTPUT);
  digitalWrite(YELLOW_LED, LOW); //LED comienza apagado
...
void loop() {
  ...
  delay(3000);             // Espera 1 minuto (60,000 milisegundos)
